...
def checkCounts(n, hallsPerCinema, sessionsPerHall):
    ...
    if counts.get("viewer", 0) != n:
        raise RuntimeError("viewer count mismatch")
    if counts.get("movie", 0) != n:
        raise RuntimeError("movie count mismatch")
    if counts.get("viewer_profile", 0) != n:
        raise RuntimeError("viewer_profile count mismatch")
    if counts.get("cinema", 0) != n:
        raise RuntimeError("cinema count mismatch")
    if counts.get("hall", 0) != n * hallsPerCinema:
        raise RuntimeError("hall count mismatch")
    if counts.get("session", 0) != n * hallsPerCinema * sessionsPerHall:
        raise RuntimeError("session count mismatch")
...
def checkForeignKeys():
    with getDbConnection() as (conn, cur):
        ...
        if cur.fetchone()[0] != 0:
            raise RuntimeError("orphan in viewer_profile")
        ...
        if cur.fetchone()[0] != 0:
            raise RuntimeError("orphan in favorite_movies")
        ...
        if cur.fetchone()[0] != 0:
            raise RuntimeError("orphan in session")
        ...
        if cur.fetchone()[0] != 0:
            raise RuntimeError("orphan in ticket")
...
def main():
    ...
    checkForeignKeys()
    print("randomDataGenerator checks passed")
